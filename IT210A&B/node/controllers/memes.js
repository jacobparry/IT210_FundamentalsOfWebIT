...
the URL by using something like '/:id/' where the : indicates that 'id' is a variable that you can then access by using req.params.id*/
    ...
    ////Post Requests for admin view
    app.post('/deleteUser', exports.deleteUser);
    app.post('/deleteImage', exports.deleteImage);
    app.post('/approveImage', exports.approveImage);
};
...
exports.memes = function(req, res,callback){ //Function exports.views will be almost exactly the same as this function, with additional filtering (images for only one user).
	...
	var options = {
		scriptPath: '/node/scripts',
	};

	PythonShell.run('memeupload.py', options, function (err, results) {
	  if (err) throw err;
...
////////////////////////////////////////////////////////////
//Lab 6b- Adding an admin page
////////////////////////////////////////////////////////////

exports.admin = function(req, res,callback){ //Function exports.views will be almost exactly the same as this function, with additional filtering (images for only one user).
...
////////////////////////////////////////////////////////////
//Lab 6b- Admin page Post controllers
////////////////////////////////////////////////////////////
...
exports.deleteImage = function (req, res, callback) {
	...
	var options = {
		scriptPath: '/node/scripts',
		args:[req.body.deleteImage],
	};

	PythonShell.run('deleteImage.py', options, function (err, results) {
	  if (err) throw err;
	  console.log('results: %j', results);
	  return res.redirect("/admin");
	});
...
exports.approveImage = function (req, res, callback) {
	...
	var options = {
		scriptPath: '/node/scripts',
		args:[req.body.approveImage],
	};

	PythonShell.run('approveImage.py', options, function (err, results) {
	  if (err) throw err;
	  console.log('results: %j', results);
	  return res.redirect("/admin");
	});
...
exports.views = function(req, res,callback)
{
	async.auto({
		getData: function get_data(callback){
			Images.findAll({where: {userId: req.params.id}}) //Function returns information for all images.  Similar to 'select * from images;'
				.then(function(getData){
					callback(null, getData); //On a successful query the results are returned in the object 'getData'
				})
				.catch(function(err){
		...
		getUser: function get_user(callback){
			Users.findAll({where: {userId: req.params.id}}) //Function returns information for all users.  Similar to 'select * from users;'
				.then(function(getUser){
					callback(null, getUser); //On a successful query the results are returned in the object 'getUser'
				})
				.catch(function(err){
